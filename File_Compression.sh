...
echo -e "Compression type: sam ${filetype_togz[*]}"
...
for file in "${arr[@]}"; do
    if [[ ! -L $file ]] && [[ -f $file ]]; then
        echo -e ">>> $(date)\nSAM file found: $file" &>>$logfile | tee -a ${file}.toBam.log
        prefix=${file%%.sam}
        samtools view -@ $threads -Shb $file -o ${prefix}.bam &>>$logfile | tee -a ${file}.toBam.log

        if [[ $? == 0 ]] && [[ ! $(grep -iP "${error_pattern}" "${file}.toBam.log") ]]; then
            rm -f $file
            echo -e "SAM-to-BAM conversion completed. New bam file: ${prefix}.bam" &>>$logfile | tee -a ${file}.toBam.log
        else
            echo -e "ERROR! SAM-to-BAM conversion failed: $file" &>>$logfile | tee -a ${file}.toBam.log
        fi
    fi
done
...
for file in "${arr[@]}"; do
    if [[ ! -L $file ]] && [[ -f $file ]]; then
        echo -e ">>> $(date)\nFiletype needed to be gzipped found: $file" &>>$logfile | tee -a ${file}.pigz.log
        pigz -p $threads -f $file &>>$logfile | tee -a ${file}.pigz.log
        if [[ $? == 0 ]] && [[ ! $(grep -iP "${error_pattern}" "${file}.pigz.log") ]]; then
            echo -e "Compression completed. New gzipped file: ${file}.gz" &>>$logfile | tee -a ${file}.pigz.log
        else
            echo -e "ERROR! Compression failed: $file" &>>$logfile | tee -a ${file}.pigz.log
        fi
    fi
done
